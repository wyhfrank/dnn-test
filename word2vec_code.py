...
def save_embeddings(file, embeddings, labels):
...
def train_data(config):
    ...
    try:
        ...
        low_dim_embs = tsne.fit_transform(final_embeddings[:plot_only, :])
        labels = [reverse_dictionary[i] for i in xrange(plot_only)]
        plot_with_labels(low_dim_embs, labels, os.path.join(config.output_path, 'tsne.png'))

    except ImportError:
        print('Please install sklearn, matplotlib, and scipy to show embeddings.')

    labels = reverse_dictionary.values()
    save_embeddings(token_embeddings_file, final_embeddings, labels)
